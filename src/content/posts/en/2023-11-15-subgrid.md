...
{% aside %}Here and in all the following examples on Codepen, I have unnecessarily placed a `div` between `<body>` and the three landmarks `<header>`, `<main>` and `<footer>`. However, those landmarks must be direct children of `<body>` [in order to be interpreted correctly by screen readers](https://webaim.org/techniques/semanticstructure/). If you want to include the following attempts somewhere, I recommend removing the `div` and set the parent grid directly on `<body>`.{% endaside %}
...
{% set slug = "zYeEJKw" %}
{% set caption = "subgrid with lines: subgrid activat" %}
{% include "partials/codepen.njk" %}
