...
{% aside %}Ich habe hier und in allen folgenden Beispielen auf Codepen unnötigerweise ein `div` zwischen `<body>` und die drei landmarks `<header>`, `<main>` und `<footer>` gesetzt. Diese müssen aber direkte Kinder von `<body>` sein, um von screen readern richtig interpretiert zu werden. Solltet ihr die folgenden Versuche irgendwo einbauen wollen, nehmt bitte das `div` raus und setzt das parent grid direkt auf `<body>`.{% endaside %}
