...
Zur besseren Veranschaulichung seiner Beispiele hebt er die unsichtbaren Rasterlinien mit gestrichelten Linien hervor. Sein Ansatz ist ähnlich wie der von Roma: Im Zusammenhang mit Pseudo-Elementen arbeitet er nicht mit `background-color`, sondern mit `border-left` (`border-right` würde auf die gleiche Weise funktionieren). Damit wird auch das Problem der sich überlappenden Gitterlinien gelöst.

Ich habe diese Lösung auf das letzte Beispiel mit den leeren Platzhaltern angewandt (ich habe die gestrichelten Linien von Josh beibehalten, um den Unterschied zur vorherigen Lösung besser sichtbar zu machen):
