...
{% aside %}Aquí y en todos los siguientes ejemplos de Codepen, he colocado innecesariamente un `div` entre `<body>` y los tres "puntos de referencia" `<header>`, `<main>` y `<footer>`. Sin embargo, esos puntos de referencia deben ser hijos directos de `<body>` [para ser interpretados correctamente por los lectores de pantalla](https://webaim.org/techniques/semanticstructure/). Si quieres incluir los siguientes intentos en algún sitio, recominedo, eliminar el `div` y establecer el grid padre directamente en `<body>`.{% endaside %}
