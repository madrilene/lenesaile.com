...
To better illustrate his examples, he emphasizes the grid lines with dashed lines. His approach is similar to Roma's: In the context of pseudo-elements he does not work with `background-color`, but with `border-left` (I'm using logical properties instead, `border-inline-end` or `border-right` would work the same way for us). It also solves the problem of overlapping grid lines.
