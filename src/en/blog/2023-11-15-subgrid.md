...
### linear-gradient without a magic number
...
Roma applies CSS pseudo-elements before the children of the `<article>` element, and after the `<article>` element itself. They are positioned absolutely, spanning the full height of their parent, with a `width` of `1px` and a blue background. The `z-index` is set to `-1`, placing them behind their parent.
...
He recommends placing empty elements on the grid instead, as any changes to the current `<article>` children positions in the grid can make the lines disappear.
...
6. [Michelle Barker introduces subgrid](https://www.youtube.com/watch?v=tueTFd2TQUA&t=2266s) within her "Creative CSS Layout" on CSS Day 2022 (YouTube)
