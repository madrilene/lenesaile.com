...
description: 'For me, subgrid was always what was missing for my mental model to fully embrace grid. In this article, I will try to create a prototype of a design that requires a four column layout with colored grid lines.'
discover:
  description: 'In this article I try to create a basic version of a design that requires a four column layout with colored grid lines, using subgrid.'
category: blogpost
key: 'subgrid'
...
For me, subgrid was always what was missing for my mental model to fully embrace grid.
...
Now the wrapper `<div>` is divided into four equally sized columns, placed directly adjacent to each other without any space between them.

{% codepen "https://codepen.io/madrilene/pen/yLZoZgE", "result" %}
subgrid with lines: default
{% endcodepen %}

The wrapper has three children, and these are now each placed in the first three columns. This is their natural behavior, because they are children of a grid, and they divide themselves into the available tracks.

### Activating subgrid

I want all three landmarks to receive the parent grid for themselves.
...
That's because they are still items of their parent grid system.
...
Now our three landmarks take over the ancestor grid and place their own children in it! Only `<main>` extends across all columns, but still only sorts its items into the first column. We'll come to that in a moment.

If I take a look at the dev tools, I can also see that three rows have been created automatically.

It is worth taking a close look at the dev tools for grid: In Firefox, when the page contains a grid with a subgrid, the entry for the subgrid is indented under its parent in the overlay grid section.
...
By default, the child elements are placed on the first available grid line.

### header

I don't have to worry about the `anchor` element in the `<header>` for now.
But according to the design, the SVG should align with the fourth grid line. I achieve this by generally aligning to the last column line:
...
While `<main>` is a subgrid, its child `<article>` is not: the default flow environment is in place. For some reason I had the feeling that I should use subgrid-values sparingly. But there is actually no reason for that. So I turn all `<article>` elements that are direct children of `<main>` into subgrids as well!
...
{% aside %}It should be noted that the selected article elements are _not_ subgrids of `<main>`, but always of the nearest ancestor element that defines a column template that is _not_ subgridded. That's our wrapper `<div>`, selected via `body > div`.{% endaside %}

Looking at the design, the `<article>`'s title and paragraph should not be placed in the normal grid flow, but each occupy its own row within the first column. I make the paragraph take up its own row within the same column.
...
The sections in `<main>` are distributed across different columns in the desktop design. I now place them explicitly in the row and column assigned to them. In our example, there are only sections within `main.landmark`, but considering future compatibility, `<main>` will accommodate further `<article>` elements in a more extended HTML.
...
I also want to place elements in their own row in the `<footer>`. I can define this extra row directly in the landmark, and in addition to the subgrid defined above, it gets its own `grid-template-rows` value.
...
That doesn't work. I _can_ display the lines in the dev tools, but I don't seem to have a way to actually address these lines with a color, using CSS. However, I have found a few approaches in the wild (In other words: Stack Overflow, I also dared to ask Chat GTP, but that was completely pointless). The approaches were not created with subgrid in mind, but trying doesn't hurt.
...
Well, that only worked halfway. The intrinsically created rows of the parent grid are colored blue, but since the subgrid containers span all columns, the gaps are no longer visible.
...
Now my parent grid has a border on the right and left, because these are the areas that are not overlaid.
...
I remembered that I could place a repeating [CSS pattern](https://projects.verou.me/css3patterns/#vertical-stripes) in the background, as the four columns are totally even. This would be like a graphic layer that only tries to replicate my grid columns.
...
That's setting a linear gradient for the `background-image` property, creating a repeating pattern of a 1-pixel blue line against a transparent background, and covering 24.95% of the container's width. Sorry for that random integer.

{% codepen "https://codepen.io/madrilene/pen/ExrvBYa", "result" %}
subgrid with lines: linear-gradient
{% endcodepen %}

It's hacky, buggy and slightly off, but good enough for now. I'm pretty sure I will still change it, as it does not produce a nice result on every viewport.

This is not a satisfactory solution - I'm hoping someone will come up with a better idea and let me know about it 😬

## Where I learned subgrid

My partner laughed when he saw me sitting in front of the computer with a technical book, old school. "[CSS - The Definitive Guide](https://www.oreilly.com/library/view/css-the-definitive/9781098117603/)" is actually where I look first when I want to learn or know something about CSS in more detail. There are over 80 pages on grid layout alone, and I highly recommend it.
